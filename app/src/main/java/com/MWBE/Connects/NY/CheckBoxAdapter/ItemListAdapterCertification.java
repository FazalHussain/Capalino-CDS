 ...
 * @author marvinlabs
 */
public class ItemListAdapterCertification extends ArrayAdapter<ListData_Agency> {
     ...
     */
    public ItemListAdapterCertification(Context context, List<ListData_Agency> items) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        try {
            if (Data.tagidlistcertification.size() > 0) {
                for (int i = 0; i < Data.tagidlistcertification.size(); i++) {
                    for (int j = 0; j < Data.tagidlistcertification_db.size(); j++) {
                        if (Data.tagidlistcertification.get(i).equalsIgnoreCase(Data.tagidlistcertification_db.get(j))) {
                            lv.setItemChecked(j, true);
                            Data.list_check_certification.set(j, true);
                        }
                    }
                }
            }
